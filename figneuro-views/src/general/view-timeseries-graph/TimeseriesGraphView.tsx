...
import { useTimeseriesInitialization, useTimeRange } from '../context-timeseries-selection'
...
const TimeseriesGraphView: FunctionComponent<Props> = ({data, width, height}) => {
    ...
    useTimeseriesInitialization(minTime, maxTime, timeOffset || 0)
    const {visibleStartTimeSeconds, visibleEndTimeSeconds } = useTimeRange(timeOffset || 0) // timeOffset is subtracted from start and end after getting from the global state

    const margins = useTimeseriesMargins()
    ...
    const plotSeries = useMemo(() => {
        ...
        if ((visibleStartTimeSeconds === undefined) || (visibleEndTimeSeconds === undefined)) {
            return plotSeries
        }
        resolvedSeries.forEach(rs => {
            let filteredTimeIndices: number[] = rs.t.flatMap((t: number, ii: number) => (visibleStartTimeSeconds <= t) && (t <= visibleEndTimeSeconds) ? ii : [])
        ...
        return plotSeries
    }, [visibleStartTimeSeconds, visibleEndTimeSeconds, resolvedSeries])
    ...
    const pixelTransform = use2dScalingMatrix({
        ...
        dataXMin: visibleStartTimeSeconds,
        dataXMax: visibleEndTimeSeconds,
        dataYMin: minValue,
        dataYMax: maxValue
    })
