let offset = 0 // so that we change which is the 0th node
let stride = 1
let num: number | undefined = undefined // so that we change the periodicity

export const getNodeColor = (nodeColorIndex: any) => {
    if (num === undefined) num = colors.length
    const i = (offset + nodeColorIndex * stride) % num
    return colors[i]
}

export const redistributeNodeColors = () => {
    if (num === undefined) num = colors.length
    offset += 1 // so that we change which is the 0th node
