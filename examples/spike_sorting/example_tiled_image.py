...
def example_tiled_image(*, height=500):
    ...
    layer1 = ssv.TiledImageLayer(label='layer 1', data=y)
    y2 = y
    y2[:, :, 0] = 0
    layer2 = ssv.TiledImageLayer(label='layer 2', data=y2)
    view = ssv.TiledImage(tile_size=tile_size, layers=[layer1, layer2], height=height)
    return view
